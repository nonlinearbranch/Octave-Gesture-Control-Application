...
const STATIC_ENGINE_GESTURES = {
  'st-fist-play-pause': 'Fist',
  'st-thumbs-up-mute': 'Thumb',
  'st-v-sign-next-prev': 'V Sign',
  'st-ok-sign-confirm': 'OK Sign',
  'st-mode-switch-cycle': 'Three Fingers',
  'st-drs-frame-power': 'DRS T-Frame'
}

const clamp = (value, min, max) => Math.max(min, Math.min(max, value))

const buildEngineSettingsPayload = (settings, devices) => {
  const cameraIndex = Math.max(
    0,
    devices.cameras.findIndex((device) => device.deviceId === settings.selectedCameraId)
  )
  const micFoundIndex = devices.microphones.findIndex(
    (device) => device.deviceId === settings.selectedMicId
  )
  const micIndex = settings.selectedMicId ? Math.max(0, micFoundIndex) : -1

  const sensitivity = clamp(Number(settings.gestureSensitivity) || 70, 10, 100)
  const normalized = sensitivity / 100
  const staticConfidenceThreshold = Number((0.88 - normalized * 0.45).toFixed(3))
  const intentSafetyThreshold = Number((0.75 - normalized * 0.4).toFixed(3))

  return {
    camera_index: cameraIndex,
    voice_enabled: Boolean(settings.selectedMicId),
    voice_input_index: micIndex,
    static_confidence_threshold: clamp(staticConfidenceThreshold, 0.3, 0.95),
    intent_safety_threshold: clamp(intentSafetyThreshold, 0.2, 0.95),
    system_action_cooldown_sec: clamp((Number(settings.actionCooldownMs) || 450) / 1000, 0.1, 5),
    ui_preferences: {
      confirmBeforeDelete: Boolean(settings.confirmBeforeDelete),
      openMonitoringAfterAdd: Boolean(settings.openMonitoringAfterAdd),
      notifications: Boolean(settings.notifications)
    }
  }
}

const buildEngineMappingPayload = (gestures) => {
  const disabledStatic = gestures
    .filter((gesture) => gesture.locked && gesture.enabled === false)
    .map((gesture) => STATIC_ENGINE_GESTURES[gesture.id])
    .filter(Boolean)

  const voiceActions = {}
  gestures
    .filter((gesture) => !gesture.locked && gesture.type === 'voice')
    .forEach((gesture) => {
      const phrase = String(gesture.phrase || '').trim().toLowerCase()
      if (!phrase) return
      // Safe default until custom action mapping UI is connected to backend action schema.
      voiceActions[phrase] = 'Click'
    })
...
function App() {
  ...
  const lastEngineMappingJsonRef = useRef('')
  ...
  useEffect(() => {
    ...
    const syncEngineStatus = async () => {
      ...
      try {
        ...
        if (!cancelled && result?.ok && result.status) {
          setEngineStatus((prev) => ({ ...prev, ...result.status }))
        }
      } catch {
        // ignore initial status sync failures
      }
    }

    const syncEngineHealth = async () => {
      if (!window.api?.getEngineHealth) return
      try {
        const result = await window.api.getEngineHealth()
        if (!cancelled && result?.ok) {
          setEngineHealth(result.health || null)
          if (result.status) setEngineStatus((prev) => ({ ...prev, ...result.status }))
        }
      } catch {
    ...
    const unsubs = []
    if (window.api?.onEngineStatus) {
      unsubs.push(
        window.api.onEngineStatus((payload = {}) => {
          if (cancelled) return
          setEngineStatus((prev) => ({ ...prev, ...payload }))
        })
      )
    }
    if (window.api?.onEngineRuntime) {
      unsubs.push(
        window.api.onEngineRuntime((payload = {}) => {
          if (cancelled) return
          setEngineRuntime(payload)
        })
      )
    }
    if (window.api?.onEngineError) {
      unsubs.push(
        window.api.onEngineError((payload = {}) => {
          if (cancelled) return
          const nextError = payload.error || payload.traceback || payload.stage || 'Engine error'
    ...
    if (window.api?.onEngineVoice) {
      unsubs.push(
        window.api.onEngineVoice((payload = {}) => {
          if (cancelled) return
          setEngineRuntime((prev) => ({
            ...(prev || {}),
            voice_phrase: payload.phrase || prev?.voice_phrase || null
          }))
        })
      )
    }
    if (window.api?.onEngineStderr) {
      unsubs.push(
        window.api.onEngineStderr((payload = {}) => {
          if (cancelled) return
          const text = String(payload.text || '').trim()
          if (!text) return
          setEngineStderr(text)
        })
      )
    }

    void syncEngineStatus()
    void syncEngineHealth()
    return () => {
      cancelled = true
  ...
  const engineSettingsPayload = useMemo(
    () => buildEngineSettingsPayload(settings, devices),
    [settings, devices]
  )
  const engineMappingPayload = useMemo(() => buildEngineMappingPayload(gestures), [gestures])

  useEffect(() => {
    if (tab !== 'live-monitoring') return
    if (!window.api?.startEngine) return
    let cancelled = false
    ...
    const ensureEngineRunning = async () => {
      try {
        ...
        setEngineStatus((prev) => ({
          ...
          phase: 'error',
          running: false,
          lastError: error?.message || 'Failed to start engine'
        }))
      }
    }
  ...
  useEffect(() => {
    if (!window.api?.updateEngineSettings) return
    const payloadJson = JSON.stringify(engineSettingsPayload)
    if (payloadJson === lastEngineSettingsJsonRef.current) return
    lastEngineSettingsJsonRef.current = payloadJson

    let cancelled = false
    const timer = setTimeout(async () => {
      try {
        const result = await window.api.updateEngineSettings(engineSettingsPayload)
        if (cancelled) return
        if (result?.status) setEngineStatus((prev) => ({ ...prev, ...result.status }))
      } catch {
  ...
  useEffect(() => {
    if (!window.api?.updateGestureMapping) return
    const payloadJson = JSON.stringify(engineMappingPayload)
    if (payloadJson === lastEngineMappingJsonRef.current) return
    lastEngineMappingJsonRef.current = payloadJson

    let cancelled = false
    const timer = setTimeout(async () => {
      try {
        await window.api.updateGestureMapping(engineMappingPayload)
      } catch {
        if (cancelled) return
        // mapping sync is best-effort
      }
    }, 220)
  ...
  // [MODIFIED] Camera handled by Python backend
  useEffect(() => {
    if (tab === 'live-monitoring') {
      // We rely on engineStatus to know if backend is ready.
      // The visuals are handled by the <img src="..." /> component.
      setCameraStatus(engineStatus.running ? 'connected' : 'connecting')
    } else {
      setCameraStatus('idle')
    }
  }, [tab, engineStatus.running])
  ...
  const systemStatus = useMemo(() => {
    if (mediaError || engineStatus.phase === 'error') return { label: 'System Error', tone: 'error' }
    if (tab === 'live-monitoring') {
      if (
        engineStatus.phase === 'starting' ||
        cameraStatus === 'connecting' ||
        micStatus === 'connecting'
      ) {
        return { label: 'Starting...', tone: 'starting' }
      }
      if (engineStatus.running || cameraStatus === 'connected' || micStatus === 'connected') {
        return { label: 'System Active', tone: 'active' }
      }
    }
    return { label: 'System Idle', tone: 'idle' }
  }, [cameraStatus, engineStatus.phase, engineStatus.running, mediaError, micStatus, tab])

  const engineMissingModules = useMemo(() => {
    const modules = engineHealth?.modules || {}
  ...
  const startTrainingSession = async (gesture, returnTab = null) => {
    ...
    try {
      const result = await window.api.startTraining({
        ...
        locked: Boolean(gesture.locked),
        action: gesture.type === 'voice' ? 'Click' : 'Click',
        targetSamples: gesture.type === 'voice' ? 0 : 140
      })
      if (result?.ok) {
        if (result.status) {
          setEngineStatus((prev) => ({ ...prev, ...result.status }))
  ...
  const finishTrainingSession = async () => {
    ...
    if (trainingSession?.source === 'engine' && window.api?.completeTraining) {
      try {
        ...
        // ignore completion errors; UI state still closes gracefully
      }
    }
  ...
  const stopTrainingSession = async () => {
    ...
    if (currentSession.progress < 100 && window.api?.cancelTraining) {
      try {
        const result = await window.api.cancelTraining()
        if (result?.status) {
          setEngineStatus((prev) => ({ ...prev, ...result.status }))
        }
      } catch {
  ...
  const handleEngineControl = async (action) => {
    try {
      if (action === 'start' && window.api?.startEngine) {
        const result = await window.api.startEngine()
        if (result?.status) setEngineStatus((prev) => ({ ...prev, ...result.status }))
        return
      }
      if (action === 'stop' && window.api?.stopEngine) {
        const result = await window.api.stopEngine()
        if (result?.status) setEngineStatus((prev) => ({ ...prev, ...result.status }))
        return
      }
      if (action === 'restart') {
        if (window.api?.stopEngine) {
          const stopped = await window.api.stopEngine()
          if (stopped?.status) setEngineStatus((prev) => ({ ...prev, ...stopped.status }))
        }
        if (window.api?.startEngine) {
          const started = await window.api.startEngine()
          if (started?.status) setEngineStatus((prev) => ({ ...prev, ...started.status }))
        }
      }
    } catch (error) {
      setEngineStatus((prev) => ({
        ...
        phase: 'error',
        running: false,
        lastError: error?.message || 'Engine operation failed'
      }))
    }
  }
          ...
          {tab === 'live-monitoring' ? (
            <div className="monitor-grid">
                ...
                {trainingSession ? (
                  <div className="training-video-overlay">
                    ...
                    {trainingSession.error ? (
                      <p className="training-video-status">Training error: {trainingSession.error}</p>
                    ) : null}
                    {trainingSession.type === 'voice' ? (
              ...
              <section className="placeholder-panel">
                ...
                <p>
                  Status: {engineStatus.phase}
                  {engineStatus.running ? ' (running)' : ''}
                </p>
                <ul className="sample-meta">
                  <li>Mode: {engineRuntime?.mode || '-'}</li>
                  <li>Dynamic Family: {engineRuntime?.dynamic_family || '-'}</li>
                  <li>Dynamic Intent: {engineRuntime?.dynamic_intent || '-'}</li>
                  <li>Confidence: {engineRuntime?.confidence ?? 0}</li>
                  <li>Static Gesture: {engineRuntime?.static_gesture || '-'}</li>
                  <li>Voice Phrase: {engineRuntime?.voice_phrase || '-'}</li>
                  <li>FPS: {engineRuntime?.fps ?? 0}</li>
                </ul>
                {engineMissingModules.length > 0 ? (
                  <p className="guide-readonly-note">
                    Missing modules: {engineMissingModules.join(', ')}
                  </p>
                ) : null}
                {engineStatus.lastError ? (
                  <p className="guide-readonly-note">Last error: {engineStatus.lastError}</p>
                ) : null}
                {engineStderr ? (
                  <p className="guide-readonly-note">Engine stderr: {engineStderr}</p>
                ) : null}
                <div className="modal-actions">
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      void handleEngineControl('start')
                    }}
                  >
                    Start Engine
                  </button>
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      void handleEngineControl('restart')
                    }}
                  >
                    Restart Engine
                  </button>
